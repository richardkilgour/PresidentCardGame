...
Does this:
    Shuffles and deals (on behalf of a players)
...
class Episode:
    def __init__(self, players, positions, deck, listener_list):
        ...
        # The highest current play (Meld) for each players
    ...
    def pick_round_starter(self):
        """Decide who is the first players for the first round"""
        if self.positions:
            # Asshole goes first
    ...
    def deal(self):
        ...
        # Deal the cards, starting at players 1 (assumes that players 0 is dealing)
        i = 1
    ...
    def player_turn(self):
        ...
        # Round is over when one players remains active
        ...
        # We have more than one active players, so let the current plyer play (or fall-though)
        ...
        # Note: May be BLOCKING!!! if it waits for the players to play
        # Really it should yield to the main thread while the players thinks about it
        action = player.play()
        if action == '␆':
            # This is the players thinking (noop)
            return
        # Check if it's valid
        if self.target_meld() and action.cards and action < self.target_meld():
            # Punish the players for cheating
            raise
        ...
        if not action.cards:
            ...
            # Remember the new highest meld from that players
            ...
            # Get the next players by moving the active players to the end of the queue
    ...
    def play(self):
        ...
        if self.state == State.INITIALISED:
            ...
            # Play until the round is won (only one players remaining)
            self.player_turn()
            if len(self.active_players) == 1:
    ...
    # State is a list of INT16. Each players is 7 ints, which are a bit-mask for:
    # The Hand (54 bits), the state (2 bits), the position (4 bits) and the meld (54 bits)
    def save_state(self):
        """"Encode each hand, each (current) meld, state and positions for each players"""
        # List of np arrays
        game_state = []
        player_names = []
        player_types = []
        ...
        # Who is active? Always players 0!!!
        # active_index = gm.players.index(gm.active_players[0])
        game_state[0][2, 27] = 1
