...
class PlayerHolder(PlayerSplitter):
    def play(self):
        ...
        if candidate.cards and self.memory.get_highest_remaining() == candidate.cards[0].value:
        ...
        for x in range(0,lowest_card.value()):
        ...
        for x in range(highest_card.value() + 1, 14):
            logging.debug("{} knows there are {} x {} out there".format(self.name, self.memory.get_number_remaining(x), PlayingCard(x * 4)))
        ...
        if (self._hand[-1].value() == candidate.cards[0].value()):
            save_it = True
            lowest_card = self._hand[0]
            for x in range(0,lowest_card.value()):
                if self.memory.get_number_remaining(x) > 0:
                    ...
                    break
            if save_it:
                for x in range(candidate.cards[0].value()+1, 14):
                    if self.memory.get_number_remaining(x) > 0:
                        # Don't risk it!
